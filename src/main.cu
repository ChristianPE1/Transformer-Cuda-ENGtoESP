...
int main()
{
    try
    {
        ...
        if (!test_batch.empty())
        {
            ...
            int epochs = 10;  // Reducir a 5 para pruebas rápidas
            int batch_size = 8;   // Batch aún más pequeño para updates más frecuentes
            float learning_rate = 0.05f;  // Learning rate más agresivo
